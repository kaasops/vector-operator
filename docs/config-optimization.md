...
- **Per-source `data_dir` is not migrated.** Migration assumes the operator-managed global data dir (mounted at `/vector-data-dir`). A `kubernetes_logs` source that overrides `data_dir` keeps its checkpoints in a separate tree the merger does not scan, so that source re-reads its retained files on a mode switch (the pre-migration outcome, no loss). The operator never sets a per-source `data_dir`; this only applies if you set one yourself.
